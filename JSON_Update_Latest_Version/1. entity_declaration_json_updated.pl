...
noun([mode:gen, num:N, type:entity, pos:subj, sem:Sem]) -->         
  {lexicon([cat:noun, wform:WForm, num:N, type:entity, stype:ent_dec, pos:P, arg:_X, sem:Sem])}, 
  WForm. 
...
lexical_rule([cat:noun, num:sg, type:entity, pos:subj, sem:Sems], List1, List2) :-  
   ...
   assert(lexicon([cat:noun, wform:WForm, num:sg, type:entity, stype:ent_dec, pos:subj, arg:_, sem:Sems])), % for entity declaration
   assert(lexicon([cat:noun, wform:DWForm, num:sg, type:entity, stype:const_dec, pos:_, arg:_, sem:Sems])), % for constraint declaration both subject and object are small
   assert(lexicon([cat:noun, wform:WForm, num:sg, type:entity, stype:rel_dec, pos:subj, arg:_, sem:Semo])), % for relationship declaration subject
   assert(lexicon([cat:noun, wform:DWForm, num:sg, type:entity, stype:rel_dec, pos:obj, arg:_, sem:Semo])), % for relationship declaration object
   assert(lexicon([cat:noun, wform:WLp, num:pl, type:entity, stype:const_dec, pos:obj, arg:_, sem:Semo])).  % for constraint declaration plural case 
