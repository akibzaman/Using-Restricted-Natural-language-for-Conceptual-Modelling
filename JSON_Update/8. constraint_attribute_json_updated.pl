...
s([type:const_dp, sem:Sem]) -->
   np([num:N, type:const_dp, pos:obj, arg:X, scope:Sco, sem:Sem]),
   vp([crd:'+', num:N, type:const_dp, arg:X, sem:Sco]),
   ['.'].

np([num:N, type:const_dp, pos:P, arg:X, scope:Sco, sem:Sem]) -->
   qnt([num:N, type:const_dp, pos:subj, arg:X, restrictor:json(['Atom'=Res]), scope:Sco, sem:Sem]),
   noun([num:sg, type:const_dp, pos:P, arg:X, sem:Res]).
   
np([num:N, type:const_dp, pos:P, arg:Y, scope:Sco, sem:Sem]) -->
   cst([num:N, type:const_dp, pos:P, arg:Y, restrictor:Res, scope:Sco, sem:Sem]),
   noun([num:N, type:const_dp, arg:Y, sem:Res]).
   
noun([num:N, type:const_dp, pos:P, arg:X, sem:Res]) -->
   {lexicon([cat:noun, wform:WForm, num:N, type:entity, pos:P, arg:X, sem:Res])}, WForm.
  
noun([num:N, type:const_dp, arg:Y, sem:Res]) -->
  {lexicon([cat:noun, wform:WForm, num:N, type:attribute, pos:obj, dt:DT, arg:Y, sem:Res])}, WForm.
...
vp([crd:'+', num:N, type:const_dp, arg:X, sem:Sco]) -->   
    v([num:N, type:const_dp, arg:X, arg:Y, sem:Sem]), 
    np([num:_, type:const_dp, pos:obj, arg:Y, scope:Sem, sem:Sco]).
...
v([num:N, type:const_dp, arg:X, arg:Y, sem:json(['Rel'=relation, 'Ind'=own, 'Var'=[X,Y]])]) -->
   [owns].
v([num:N, type:const_dp, arg:X, arg:Y, sem:json(['Rel'=relation, 'Ind'=consist_of, 'Var'=[X,Y]])]) -->
   [consists, of].

qnt([num:N, type:const_dp, pos:subj, arg:X, restrictor:JsonHead, scope:JsonBody, 
					sem:json(['Forall'=json(['Var'=X, 
						'Implies'= json([head=JsonHead, body=JsonBody])])])]) --> ['Every'].
   
cst([num:N, type:const_dp, pos:obj, arg:X, restrictor:Res, scope:Sco, 
					sem:json(['Var'=X,
                    'And'=json(['Atom'=[ Sco,
                                         json(['Rel'=min,
                                               'Var'=X,
